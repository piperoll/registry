# PIR-2026-0017 - Moltbook misconfigured database exposes ~1.5M agent API keys with unauthenticated read/write
...
### The failure
- `root_cause`: primary `credential-exposure`; contributing `operator-error` (misconfiguration)
...
- `sources`: wiz.io/blog/exposed-moltbook-database-reveals-millions-of-api-keys (primary, researcher); thecyberexpress.com/moltbook-platform-exposes-1-5-mn-api-keys/ (independent); infosecurity-magazine.com/news/moltbook-exposes-user-data-api/ (independent); bankinfosecurity.com/moltbook-gave-everyone-control-every-ai-agent-a-30710 (independent). Independence: good - primary researcher plus multiple independent outlets.
...
2. Severity reclassified degraded -> near-miss: full exposure with zero confirmed realized loss matches the schema's near-miss definition and the PIR-2026-0001 precedent for exposed-but-unabused credentials. The v0.1 `exploitation_status` field now carries the found-in-production distinction the candidate's "degraded" was trying to express.
