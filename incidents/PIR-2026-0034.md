# PIR-2026-0034 - Gemini CLI hallucinates a successful mkdir, then overwrite-destroys a user's files via Windows move semantics
...
### The failure
- `root_cause`: primary `plain-error` (acted on a hallucinated world state); contributing `tool-error` (harness performs no ground-truth verification of command results)
...
- `sources`: medium.com/@geek.with.a.camera/i-let-google-gemini-cli-move-my-files-it-apologized-after-deleting-them-beec518342c3 (primary, first-party); github.com/google-gemini/gemini-cli/issues/4586 (independent report, same failure class); developers.slashdot.org/story/25/07/26/0642239/google-gemini-deletes-users-files-then-just-admits-i-have-failed-you-completely-and-catastrophically; winbuzzer.com/2025/07/26/googles-gemini-cli-deletes-user-files-confesses-catastrophic-failure-xcxwbn/; incidentdatabase.ai/cite/1178/. Independence: medium - the narrative chains to the victim's own account; the failure class is corroborated by unrelated users in the issue tracker.
